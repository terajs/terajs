...
export interface GeneratedRouteConfigInput {
  filePath: string;
  path?: string;
  layout?: string;
...
function buildPathFromNode(ancestors: RouteNode[], node: RouteNode): string {
  const segments = [...ancestors, node]
    ...
    .map((entry) => segmentToPath(stripExtension(entry.name)));

  const builtPath = segments
    .filter((segment) => segment !== "")
    .join("/");

  return builtPath === "" ? "/" : `/${builtPath}`;
}
...
function generateRoutesFromNode(
  ...
  routeConfigMap: Map<string, GeneratedRouteConfigInput> = new Map()
): string[] {
  ...
  const children = Array.from(node.children.values()).sort((left, right) => {
    const leftName = left.name === "index" ? "" : left.name;
    const rightName = right.name === "index" ? "" : right.name;
    return sortSegments(leftName, rightName);
  });
...
export function generateRouteConfigWithAssets(
  files: string[],
  manifest?: Record<string, any>,
  routeConfigs: GeneratedRouteConfigInput[] = []
): string {
  const tree = buildRouteTree(files);
  const configMap = new Map<string, GeneratedRouteConfigInput>();
