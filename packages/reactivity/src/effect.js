 ...
 */
import { pushEffect, popEffect, currentEffect } from "./deps.js";
import { isServer } from "./dx/runtime.js";
import { shouldBatch, queueEffect } from "./dx/batch.js";
import { Debug, createReactiveMetadata, getCurrentComposable, getCurrentContext, removeDependencyNode } from "@terajs/shared";
...
export function effect(fn, scheduler) {
    ...
    effectFn._meta = debugInstrumentationEnabled
...
function cleanup(effectFn) {
    if (debugInstrumentationEnabled) {
        Debug.emit("effect:cleanup", {
            effect: effectFn,
            owner: effectFn._owner,
            context: effectFn._context,
            composable: effectFn._meta?.composable
        });
    }
    // NEW: remove graph edges for this effect
    const meta = effectFn._meta;
...
function disposeEffect(effectFn) {
    ...
    cleanup(effectFn);
    if (effectFn.cleanups.length) {
        for (const fn of effectFn.cleanups)
            fn();
        effectFn.cleanups.length = 0;
    }
    if (effectFn.children?.length) {
