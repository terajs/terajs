...
import { createUIKitWireTransport, type UIKitWireTransport } from "./wireTransport.js";

export interface UIKitGeneratedCompiledModule {
...
export interface UIKitGeneratedRouteRecord {
  id: string;
  path: string;
  filePath: string;
  layouts: Array<{
...
export interface CreateUIKitGeneratedRouteTransportOptions {
  modules: UIKitGeneratedCompiledModule[];
  routes: UIKitGeneratedRouteRecord[];
  initialPath?: string;
  transport?: UIKitWireTransport;
}

export interface UIKitGeneratedRouteTransport {
  route: UIKitGeneratedRouteRecord;
  transport: UIKitWireTransport;
}
...
export function createUIKitGeneratedRouteTransport(
  options: CreateUIKitGeneratedRouteTransportOptions,
): UIKitGeneratedRouteTransport {
  const route = options.routes.find((candidate) => candidate.path === (options.initialPath ?? "/"))
    ?? options.routes[0];
  if (!route) {
    throw new Error("UIKit generated route transport requires at least one route.");
  }

  const pageModule = options.modules.find((module) => module.filePath === route.filePath);
  if (!pageModule) {
    throw new Error(`Missing UIKit generated page module for route ${route.path}.`);
  }

  const transport = options.transport ?? createUIKitWireTransport();
  ...
  function getSetupExecutor(module: UIKitGeneratedCompiledModule) {
  ...
  function renderGeneratedModule(module: UIKitGeneratedCompiledModule, input?: unknown): unknown {
