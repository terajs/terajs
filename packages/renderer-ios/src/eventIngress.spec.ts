...
describe("renderer-ios native event ingress", () => {
  ...
  it("enforces maximum text length on native text input events before syncing state", () => {
    ...
    const rendered = session.mountIRNode(node, { value, maxLength, onInput }) as UIKitBridgeElementNode;
    const input = session.root.children[0] as UIKitNativeViewNode;

    session.dispatchNativeEvent(rendered.id, "input", { value: "BetaGamma" });
  ...
  it("enforces text limits and keeps selection aligned during composition", () => {
    ...
    const rendered = session.mountIRNode(node, { value, maxLength, onCompositionUpdate, onCompositionEnd }) as UIKitBridgeElementNode;
    const input = session.root.children[0] as UIKitNativeViewNode;
