import type { UIKitBridgeCommand, UIKitNativeEventPacket } from "./bridgeContracts.js";

export type UIKitTransportValue =
  | null
  | boolean
  | number
  | string
  | UIKitTransportValue[]
  | { [key: string]: UIKitTransportValue };
...
function normalizeTransportValue(
  ...
  const result: Record<string, UIKitTransportValue> = {};
...
function normalizeBridgeCommand(command: unknown, path: string): UIKitBridgeCommand {
  if (!isRecord(command)) {
    throw new Error(`${path} must be an object`);
  }
  ...
  switch (type) {
    ...
    default:
      throw new Error(`${path}.type is not a supported UIKit bridge command`);
  }
}

function normalizeNativeEventPacket(packet: unknown, path: string): UIKitNativeEventPacket {
  if (!isRecord(packet)) {
    throw new Error(`${path} must be an object`);
  }
...
export function stringifyUIKitBridgeCommands(commands: readonly UIKitBridgeCommand[]): string {
  return JSON.stringify(commands.map((command, index) => normalizeBridgeCommand(command, `commands[${index}]`)));
}

export function parseUIKitBridgeCommands(input: string | unknown): UIKitBridgeCommand[] {
  const parsed = parseJsonInput(input);
  if (!Array.isArray(parsed)) {
    throw new Error("UIKit bridge commands must be an array");
  }

  return parsed.map((command, index) => normalizeBridgeCommand(command, `commands[${index}]`));
}

export function stringifyUIKitNativeEventPacket(packet: UIKitNativeEventPacket): string {
  return JSON.stringify(normalizeNativeEventPacket(packet, "packet"));
}

export function parseUIKitNativeEventPacket(input: string | unknown): UIKitNativeEventPacket {
