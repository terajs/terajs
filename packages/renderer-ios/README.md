...
- package-local bridge normalization now translates standard props and events such as `aria-label`, input hints, structured selection-range and caret props, textarea line-count and viewport props, secure input traits, keyboard and correction hints, image alt text, and `click` into native-facing UIKit names
