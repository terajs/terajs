...
- package-local native event ingress now normalizes beforeinput replacement previews, delete-style payloads, multiline replacement ranges, text-input change, and text-limit-aware composition replacement previews with selection state, alongside structured text-selection payloads and switch change events, and syncs native state into the host-session proof tree
