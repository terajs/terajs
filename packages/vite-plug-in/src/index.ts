...
function terajsPlugin(options: TerajsVitePluginOptions = {}): Plugin {
  ...
  function generateAppModule() {
    ...
    return [
      ...
      `}`,
      `function hasExplicitRouterLink(link) {`,
      `  return link.hasAttribute('router-link') || link.hasAttribute('data-router-link');`,
      `}`,
      ...
      `  });`,
      `}`,
      `function resolveRouterLinkTarget(event) {`,
      ...
      `    return null;`,
      `  }`,
      `  const explicit = hasExplicitRouterLink(link);`,
      `  if (!explicit && !ROUTER_LINK_INTERCEPTION.enabled) {`,
      `    return null;`,
      `  }`,
      ...
      `    };`,
      `    const onClick = (event) => {`,
      `      const href = resolveRouterLinkTarget(event);`,
      `      if (!href) {`,
      `        return;`,
      `      }`,
