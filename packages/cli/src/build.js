...
import terajsPlugin, { getWorkspaceConfig } from "@terajs/app/vite";
import { buildNativeTarget } from "./nativeBuild.js";
const TARGET_ORDER = ["web", "android", "ios"];
...
function formatRelativePath(cwd, targetPath) {
    const relative = path.relative(cwd, targetPath);
    return relative.length > 0 ? relative.replace(/\\/g, "/") : ".";
}
function hasProjectViteConfig(cwd, hasFile) {
    return VITE_CONFIG_FILES.some((fileName) => hasFile(path.join(cwd, fileName)));
}
export function collectBuildTarget(value, previous = []) {
    const parts = value
        .split(",")
...
export function parseBuildTargets(values) {
...
export function resolveBuildTargets(workspace, requestedTargets) {
    ...
    for (const target of selectedTargets) {
        if (!configuredTargets.has(target)) {
            throw new Error(`Target "${target}" is not enabled in workspace.targets.selected. Update terajs.config or choose one of: ${workspace.targets.selected.join(", ")}.`);
        }
    }
    return TARGET_ORDER.filter((target) => selectedTargets.includes(target));
}
export function createBuildPlan(workspace, requestedTargets) {
...
export async function buildWebTarget(step, dependencies) {
...
export async function executeBuildPlan(plan, dependencies) {
    const cwd = dependencies.cwd ?? process.cwd();
