...
- package-local bridge normalization now translates standard props and events such as `aria-label`, input placeholders, secure input traits, keyboard and correction hints, image alt text, and `click` into native-facing Android names
...
- package-local native event ingress now normalizes text-input and switch change events and syncs native state into the host-session proof tree
