...
- package-local native event ingress for beforeinput replacement previews, transfer payloads, delete-style payloads, multiline replacement ranges, text input text, text-limit-aware composition start or update previews with selection state, structured text selection payloads, and switch state so host-session state and JS handlers stay aligned
