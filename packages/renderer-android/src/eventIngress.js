import { createAndroidBeforeInputPayload, extractAndroidBeforeInputState, } from "./beforeInputEventPayload.js";
import { createAndroidCompositionPayload, extractAndroidCompositionState, } from "./compositionEventPayload.js";
 ...
 */
export function ingestAndroidNativeEvent(bridgeNode, nativeNode, name, payload) {
    ...
    if (AndroidTextInputViewTypes.has(bridgeNode.viewType) && AndroidBeforeInputEventNames.has(normalizedName)) {
        const beforeInput = applyAndroidTextEventConstraints(bridgeNode.props, extractAndroidBeforeInputState(bridgeNode.props, payload));
    ...
    if (AndroidTextInputViewTypes.has(bridgeNode.viewType) && normalizedName === "change") {
        ...
        if (value != null) {
            const constrainedText = applyAndroidTextEventConstraints(bridgeNode.props, {
                text: value
            }).text ?? value;
    ...
    if (AndroidTextInputViewTypes.has(bridgeNode.viewType) && AndroidCompositionEventNames.has(normalizedName)) {
        const composition = applyAndroidTextEventConstraints(bridgeNode.props, extractAndroidCompositionState(bridgeNode.props, normalizedName, payload));
