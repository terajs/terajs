...
function extractRangeFromSelectionValue(value: unknown): AndroidNativeSelectionRange | undefined {
  if (typeof value === "number" && Number.isFinite(value)) {
    return {
      start: value,
      end: value
...
export function extractAndroidSelectionRange(payload: unknown): AndroidNativeSelectionRange | undefined {
  return extractRangeFromSelectionValue(payload);
}

export function createAndroidSelectionPayload(range: AndroidNativeSelectionRange, payload: unknown): unknown {
  const selection = {
    start: range.start,
    end: range.end
  };
