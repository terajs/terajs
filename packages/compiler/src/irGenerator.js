 ...
 */
export function generateIRModule(sfc) {
    // Normalize template into a string
    const templateSource = typeof sfc.template === "string"
        ? sfc.template
...
function areStaticProps(props) {
    return props.every((prop) => prop.kind === "static");
}
function isStaticIRNode(node) {
    return node.flags?.static === true;
}
function getBindingHint(value) {
 ...
 */
function normalizeNode(node, scopeId) {
    const base = {
        type: node.type,
        loc: undefined,
        flags: {}
