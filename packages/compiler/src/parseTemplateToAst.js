...
import { tokenizeTemplate } from "./templateTokenizer.js";
export function parseTemplateToAst(template) {
...
class ParserContext {
        ...
        while (!this.eof()) {
            ...
            if (token.type === "tagOpen") {
                ...
                if (attrToken?.type === "attrName" && (attrToken.value === "v-else" || attrToken.value === "v-else-if")) {
                    ...
                    const elseNode = this.parseElement();
    ...
    finalizeElement(tag, props, children, ifDir, forDir) {
        if (tag === "Portal") {
